...
describe("[Challenge] ABI smuggling", function () {
  ...
  it("Execution", async function () {
    ...
    let testSelfAuthorizedVault = await (
      await ethers.getContractFactory("testSelfAuthorizedVault", player)
    ).deploy(token.address, vault.address, recovery.address);
    let data = await testSelfAuthorizedVault.start();
