...
describe("[Challenge] Wallet mining", function () {
  ...
  it("Execution", async function () {
    ...
    // let sigleton = "0x34cfac646f301356faa8b21e94227e3583fe3f5f";
    // let masterCopy = await (
    //   await ethers.getContractFactory("GnosisSafe", player)
    // ).deploy();
    // console.log("masterCopy", masterCopy.address);
    // let walletFactory = await (
    //   await ethers.getContractFactory("GnosisSafeProxyFactory", player)
    // ).deploy();
    // console.log("walletFactory", walletFactory.address);
    // await walletFactory.createProxyWithNonce(sigleton,);
    // for (let i = 0; i < 100; i++) {
    //   let aim = await walletDeployer.drop("0x");
    //   console.log(i, aim.toString());
    // }
    // await authorizer.init([player.address], [DEPOSIT_ADDRESS]);
    console.log(await authorizer.owner());
  });
